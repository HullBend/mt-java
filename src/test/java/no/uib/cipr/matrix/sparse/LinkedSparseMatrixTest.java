...
public class LinkedSparseMatrixTest extends SparseMatrixTestAbstract {
    ...
    public void ignoredTimedMult() {
        ...
        System.out.println("created matrices");
        Matrix sparse = new LinkedSparseMatrix(dense.numRows(), dense.numColumns());
        sparse.set(dense);

        for (Matrix m : Lists.newArrayList(dense, sparse)) {
            System.out.println("starting " + m.getClass());
            Matrix t = new DenseMatrix(m);
            t.transpose();
            Matrix o = new DenseMatrix(dense.numRows(), dense.numColumns());
            System.out.println("warming up " + m.getClass() + " " + o.getClass());
            for (int i = 0; i < 10; i++)
                m.mult(t, o);
            System.out.println("starting " + m.getClass() + " " + o.getClass());
            watch.start();
            for (int i = 0; i < 100; i++)
                m.mult(t, o);
            watch.stop();
            System.out.println(m.getClass() + " " + o.getClass() + " " + watch);
        }
    }
            ...
            for (int r = 0; r < 10; r++) {
                for (int m = 10000; m <= 100000; m = m + 10000) {
                    for (int n = 1000; n <= 10000; n = n + 1000) {
                        ...
                        String[] line = new String[] { Integer.toString(n), Integer.toString(m),
                                Long.toString(denseMem), Long.toString(denseInitTime), Long.toString(denseMultTime),
                                Long.toString(sparseMem), Long.toString(sparseInitTime),
                        ...
                        System.out.println(origA.numRows() + " " + origB.numColumns() + " " + patternA.length + " "
                                + patternB.length);
                    }
                }
            }
