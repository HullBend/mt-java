...
public final class Snaupd {
    ...
    public static void snaupd(intW intw, String s, int i, String s1, int j, floatW floatw, float af[], int k, int l,
            float af1[], int i1, int j1, int ai[], int k1, int ai1[], int l1, float af2[], int i2, float af3[], int j2,
            int k2, intW intw1) {
        label0: {
            Etime.etime();
            if (intw.val == 0) {
                ...
                if (nb <= 0)
                    nb = 1;
                if (floatw.val <= 0.0F)
                    floatw.val = Slamch.slamch("EpsMach");
                np.val = l - j;
                nev0.val = j;
                int i3 = 1;
            ...
            Snaup2.snaup2(intw, s, i, s1, nev0, np, floatw.val, af, k, mode, iupd, ishift, mxiter, af1, i1, j1, af3,
                    (ih - 1) + j2, ldh, af3, (ritzr - 1) + j2, af3, (ritzi - 1) + j2, af3, (bounds - 1) + j2, af3,
                    (iq - 1) + j2, ldq, af3, (iw - 1) + j2, ai1, l1, af2, i2, intw1);
            if (intw.val == 3)
                ai[(8 - 1) + k1] = np.val;
            if (intw.val == 99) {
