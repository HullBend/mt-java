	...
	public static void dlasv2(double d, double d1, double d2, doubleW doublew,
		...
		if (d11 == 0.0D) {
			...
			if (flag1) {
				...
				double d4 = 0.5D * (d24 + d22);
				doublew.val = d13 / d4;
				doublew1.val = d9 * d4;
				if (d20 == 0.0D) {
					if (d16 == 0.0D)
						d28 = Util.dsign(2D, d10) * Util.dsign(1.0D, d12);
					else
						d28 = d12 / Util.dsign(d8, d10) + d18 / d28;
				} else {
					d28 = (d18 / (d24 + d28) + d18 / (d22 + d16)) * (1.0D + d4);
				}
				d16 = Math.sqrt(d28 * d28 + 4D);
				d6 = 2D / d16;
				d26 = d28 / d16;
				d5 = (d6 + d26 * d18) / d4;
				d25 = ((d14 / d10) * d26) / d4;
		...
		if (byte0 == 1)
			d31 = Util.dsign(1.0D, doublew3.val)
					* Util.dsign(1.0D, doublew5.val) * Util.dsign(1.0D, d);
		if (byte0 == 2)
			d31 = Util.dsign(1.0D, doublew2.val)
					* Util.dsign(1.0D, doublew5.val) * Util.dsign(1.0D, d1);
		if (byte0 == 3)
			d31 = Util.dsign(1.0D, doublew2.val)
					* Util.dsign(1.0D, doublew4.val) * Util.dsign(1.0D, d2);
		doublew1.val = Util.dsign(doublew1.val, d31);
		doublew.val = Util.dsign(doublew.val,
				d31 * Util.dsign(1.0D, d) * Util.dsign(1.0D, d2));
	}
}
