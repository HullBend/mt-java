...
public final class Dlanst
{
    public static double dlanst(String s, int i, double ad[], int j, double ad1[], int k)
    {
        ...
        if((Lsame.lsame(s, "O") || s.regionMatches(0, "1", 0, 1)) || Lsame.lsame(s, "I"))
        {
            ...
            {
                d = Math.max(Math.abs(ad[(1 - 1) + j]) + Math.abs(ad1[(1 - 1) + k]), Math.abs(ad1[(i - 1 - 1) + k]) + Math.abs(ad[(i - 1) + j]));
                int i1 = 2;
                for(int k1 = (i - 1 - 2) + 1; k1 > 0; k1--)
                {
                    d = Math.max(d, Math.abs(ad[(i1 - 1) + j]) + Math.abs(ad1[(i1 - 1) + k]) + Math.abs(ad1[(i1 - 1 - 1) + k]));
                    i1++;
                }
