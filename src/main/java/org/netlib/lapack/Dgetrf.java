    ...
    public static void dgetrf(int i, int j, double ad[], int k, int l, int ai[], int i1, intW intw)
    {
        ...
        } else
        {
            ...
            for(int j2 = ((Math.min(i, j) - 1) + i2) / i2; j2 > 0; j2--)
            {
                ...
                if(k1 + l1 <= j)
                {
                    Dlaswp.dlaswp((j - k1 - l1) + 1, ad, (1 - 1) + ((k1 + l1) - 1) * l + k, l, k1, (k1 + l1) - 1, ai, i1, 1);
                    Dtrsm.dtrsm("Left", "Lower", "No transpose", "Unit", l1, (j - k1 - l1) + 1, 1.0D, ad, (k1 - 1) + (k1 - 1) * l + k, l, ad, (k1 - 1) + ((k1 + l1) - 1) * l + k, l);
                    if(k1 + l1 <= i)
                        Dgemm.dgemm("No transpose", "No transpose", (i - k1 - l1) + 1, (j - k1 - l1) + 1, l1, -1D, ad, ((k1 + l1) - 1) + (k1 - 1) * l + k, l, ad, (k1 - 1) + ((k1 + l1) - 1) * l + k, l, 1.0D, ad, ((k1 + l1) - 1) + ((k1 + l1) - 1) * l + k, l);
                }
                k1 += i2;
            }
